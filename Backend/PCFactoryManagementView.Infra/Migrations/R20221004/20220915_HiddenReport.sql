...
----------------------------------------------------------------------------------------
--TELA HIDDENREPORT–Relatório de Distribuição da Gravidade de Manutenção OM

IF NOT EXISTS (SELECT IDScreen FROM TBLScreen WHERE Code = 'HIDDENREPORT'
...
BEGIN
 ...
 VALUES ('HIDDENREPORT', 'HIDDENREPORT–Go To Order/Operation', 'HIDDENREPORT–Ir a Orden/Operación', 1, (SELECT IDAplication FROM TBLAplication WHERE Code = 'PCF4View'), GETDATE(), 0)
...
IF NOT EXISTS (SELECT IDOperationScreen FROM TBLOperationScreen WHERE IDScreen = (SELECT IDScreen FROM TBLScreen WHERE Code = 'HIDDENREPORT'
...
BEGIN
 ...
 (SELECT IDScreen FROM TBLScreen WHERE Code = 'HIDDENREPORT'
...
-- --------------------------------------------------------------------------------------
--TELA HIDDENREPORT–Relatório de Sumarizado de Contagem de Inventario

IF NOT EXISTS (SELECT IDProfileUserGroup FROM TBLProfileUserGroup WHERE IDOperationScreen = (

 SELECT IDOperationScreen FROM TBLOperationScreen WHERE IDScreen = (SELECT IDScreen FROM TBLScreen WHERE Code = 'HIDDENREPORT'
...
---- --------------------------------------------------------------------------------------
--IF NOT EXISTS (SELECT IDScreenMenu FROM TBLScreenMenus WHERE Code = 'VIEW_MENU_HIDDENREPORT'
...
-- VALUES ('VIEW_MENU_HIDDENREPORT', (SELECT IDScreenMenu FROM TBLScreenMenus WHERE Code = 'VIEW_MENU_MATERIAIS'), 6, 1, 'HIDDENREPORT',

-- (SELECT IDScreen FROM TBLScreen WHERE Code = 'HIDDENREPORT'

-- AND IDAplication = (SELECT IDAplication FROM TBLAplication WHERE Code = 'PCF4View')), '/report/oee/HIDDENREPORT')
...
--TELA HIDDENREPORT
IF NOT EXISTS (SELECT IDLocalization FROM TBLLocalizations WHERE TranslationKey = 'VIEW_MENU_HIDDENREPORT'

 AND IDApplication = (SELECT IDAplication FROM TBLAplication WHERE Code = 'PCF4View')

 AND IDScreen = (SELECT IDScreen FROM TBLScreen WHERE Code = 'HIDDENREPORT' AND IDAplication = (SELECT IDAplication FROM TBLAplication WHERE Code = 'PCF4View')))

BEGIN

 INSERT INTO TBLLocalizations (PTLocal, ENLocal, ESLocal, FlgEnable, FlgProduct, TranslationKey, IDApplication, IDScreen)

 VALUES ('HIDDENREPORT–Ir Para Ordem/Operação', 'HIDDENREPORT–Go To Order/Operation', 'HIDDENREPORT-Ir a Orden/Operación',1,1,'VIEW_MENU_HIDDENREPORT',
...
VALUES ('HIDDENREPORT', 'ReportTab', 'ReportTab', 'REPORT',1, 'TABLETAB',0,1,'', (SELECT IDScreen FROM TBLScreen WHERE Code = 'HIDDENREPORT' AND IDAplication =
(SELECT IDAplication FROM TBLAplication WHERE Code = 'PCF4View')))
END
GO
...
--REPORT / TIPO DE GRÁFICO OU TABELA - HIDDENREPORT
...
BEGIN
INSERT INTO TBLScreenFields (TableName, FieldName,ComponentType, GroupName, [Enabled], DefaultValue, [Hidden], FieldOrder, CustomField,IdScreen)
VALUES ('HIDDENREPORT', 'table', 'ReportChart', 'TABLETAB' , 1, 'report/oee/HIDDENREPORT ',0,1,'',
(SELECT IDScreen FROM TBLScreen WHERE Code = 'HIDDENREPORT' AND IDAplication = (SELECT IDAplication FROM TBLAplication WHERE Code = 'PCF4View')))
END
GO
