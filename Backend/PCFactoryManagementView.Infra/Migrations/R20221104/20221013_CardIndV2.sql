...
AND FieldName = 'Card de Indicadores (8x3)' 
AND GroupName = 'DashboardCard' 
AND ComponentType = 'indicatorscardv2'
AND DefaultValue = 'IndicatorsProd/v1/IndicatorsProdV2'
...
BEGIN 
	...
	VALUES ('VIEWDASHBOARD', 'Card de Indicadores (8x3)', 'DashboardCard', 'indicatorscardv2', 'IndicatorsProd/V1/IndicatorsProdV2', 0, 0, 1, 0, 0, 0, (SELECT IDScreen FROM TBLScreen WHERE Code = 'VIEWDASHBOARD'), 'carrossel')
END
GO
...
IF NOT EXISTS (SELECT IDPref FROM TBLPrefs WHERE IDScreen = (SELECT IDScreen FROM TBLScreen WHERE Code = 'VIEWDASHBOARD' 
	...
	AND [NAME] = 'Card de Indicadores (8x3)' 
...
BEGIN
	...
	'{"GridsterConfig":{"Visible":false,"Cols":3,"Rows":2,"IdCard":0,"Widget":"indicatorscardv2","Height":0.0,"Width":0.0,"ChartUrl":"PredictedProd/V1/IndicatorsProdV2","X":0,"Y":0,"idLastEditUser":1},"PrefsGrid":null}')
END
GO

EXEC SP_TBLLocalizations22 @ScreenCode = 'VIEWDASHBOARD', @TranslationKey = 'Card de Indicadores (8x3)',
@PTLocal = 'Card de Indicadores (8x3)',
@ENLocal = 'Indicators Card (8x3)',
@ESLocal = 'Card de Indicadores (8x3)',
